...
impl TimerFile {
    ...
    pub fn timerfd_settime(
        ...
        unsafe {
            ...
            // let mut u_old_value: timespec_t = timespec_t::from(duration);
    ...
    pub fn timerfd_gettime(&self, curr_value: Option<&mut itimerspec_t>) -> i32 {
        unsafe {
            let mut ret = 0;
            let duration = Duration::new(0, 0);
            let mut u_curr_value: itimerspec_t =
                itimerspec_t::new(timespec_t::from(duration), timespec_t::from(duration));
            // let mut u_curr_value: timespec_t = timespec_t::from(duration);
            let fd = self.host_fd() as FileDesc;
